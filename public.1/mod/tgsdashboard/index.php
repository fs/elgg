<?php
	 ...
	 */
		...
		if (empty($callback)) {
			...
			$area1 = elgg_view("tgsdashboard/sitemessage");

			//set a view to display newest members
			$area1 .= elgg_view("tgsdashboard/newestmembers");
			
			$area1 .= elgg_view("tgsdashboard/tagcloud");

			//set a view to display a welcome message
			$body .= elgg_view("tgsdashboard/welcome");
		...
		$nav = elgg_view('tgsdashboard/nav',array(
															'type' => $type,
															'subtype' => $subtype,
															'orient' => $orient
														));
														
		$wirepost = elgg_view('thewire/forms/add');
		
		if (empty($callback)) {
			$body .= elgg_view('tgsdashboard/container', array('body' => $wirepost . $nav . $river . elgg_view('tgsdashboard/js')));
			page_draw(elgg_echo('dashboard'),elgg_view_layout('sidebar_boxes',$area1,$body));
		} else {
			header("Content-type: text/html; charset=UTF-8");
			echo $wirepost . $nav . $river . elgg_view('tgsdashboard/js');
		}
