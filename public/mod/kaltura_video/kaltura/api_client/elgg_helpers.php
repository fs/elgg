...
function kaltura_get_metadata($entity) {
	...
	if(isloggedin() && get_plugin_setting("alloweditor","kaltura_video") != 'no') {
		if($entity->canEdit()) {
	...
	if(get_plugin_setting("enablerating","kaltura_video") == 'no') $ob->kaltura_video_rating_on = 'Off';
...
function kaltura_update_object(&$entry,$kmodel=null,$access=null,$user_guid=null,$container_guid=null,$force=false) {
	...
	if($kmodel) {
		...
		$default_player = get_plugin_setting('defaultplayer', 'kaltura_video');
		if($default_player == 'custom') {
			$widgetUi = get_plugin_setting('custom_kdp', 'kaltura_video');
		}
		else {
...
function kaltura_get_error_page($code='',$text = "",$popup=true) {
	...
	$password = get_plugin_setting('password', 'kaltura_video');
...
function kaltura_create_generic_widget_html ( $entryId , $size='l' , $version=null , $version_kshow_name=null , $version_kshow_description=null)
{
    ...
    if($default_player == 'custom') {
		$widgetUi = get_plugin_setting('custom_kdp', 'kaltura_video');
		$w  = get_plugin_setting('custom_kdp_width', 'kaltura_video');
		$h  = get_plugin_setting('custom_kdp_height', 'kaltura_video');
		if($size != 'm') {
			if($w && $h) {
				$width = $w;
				$height = $h;
...
function kaltura_view_select_privacity($video_id,$access_id,$group_mode=false,$collaborative=false) {
	global $CONFIG;
	$ret = elgg_view('input/access', array('internalname' => 'access_id', 'class'=>'input-access ajaxprivacity','internalid'=>'ID'.$video_id, 'value' => $access_id));

	if($group_mode && get_plugin_setting("alloweditor","kaltura_video")!='no') {
