...
class KalturaModel {
	...
	function getAdminSession($privileges = "")
    {
        ...
        $partnerId = get_plugin_setting('partner_id', 'kaltura_video');
        $secret = get_plugin_setting('admin_secret', 'kaltura_video');
...
class KalturaHelpers
{
	...
	function getServerUrl()
    {
		...
		$type = get_plugin_setting('kaltura_server_type', 'kaltura_video');
		$url = get_plugin_setting('kaltura_server_url', 'kaltura_video');
	...
	function getContributionWizardFlashVars($ks, $entryId = null)
	{
		...
		$flashVars["partnerId"] 	= get_plugin_setting("partner_id","kaltura_video");
		$flashVars["subPartnerId"] 	= get_plugin_setting("subp_id", "kaltura_video");
		if ($entryId)
    		$flashVars["kshowId"] 		= "entry-".$entryId;
		else
			$flashVars["kshowId"] 		= "-2";
		$flashVars["afterAddentry"] = "editVideo";
	...
	function getPlayerByType($type)
	{
		...
		{
			//$player = $players[get_plugin_setting("kaltura_default_player_type","kaltura_video")];
			$player = $players['grey'];
		}
	...
	function getSwfUrlForBaseWidget($uiConf)
	{
		...
		return KalturaHelpers::getServerUrl() . "/index.php/kwidget/wid/_" . get_plugin_setting("partner_id","kaltura_video") . "/ui_conf_id/" . $uiConf;
	}

	function getSwfUrlForWidget($widgetId = null, $uiConfId = null)
	{
	    if (!$widgetId)
	        $widgetId = "_" . get_plugin_setting("partner_id","kaltura_video");
