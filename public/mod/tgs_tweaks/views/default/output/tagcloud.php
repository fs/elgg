...
if (!empty($vars['tagcloud']) && is_array($vars['tagcloud'])) {
	...
	foreach($vars['tagcloud'] as $tag) {
		...
		$cloud .= "<a href=\"" . $vars['url'] . "search/?tag=". urlencode($tag->tag) . $object . $subtype . "&search_type=tags\" style=\"font-size: {$size}%\" title=\"".addslashes($tag->tag)." ({$tag->total})\" style=\"text-decoration:none;\">" . htmlentities($tag->tag, ENT_QUOTES, 'UTF-8') . "</a>";
	}
	echo "<div class='tagcloudWrapper'>";
	echo $cloud;
	echo "</div>";
}