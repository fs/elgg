...
import com.elggmediaplayer.utils.*;
import com.elggmediaplayer.feeds.FeedListener;


class com.elggmediaplayer.players.ControlbarView extends AbstractView 
	...
	private function setColorsClicks() {
		...
		tgt.scrub.icn.red._visible = false; /* initially hide playback head fill */
		
		tgt.scrub.icn.onRollOver = function() { 
			tgt.scrub.icn.red._visible = true;
		};
		tgt.scrub.icn.onRollOut = function() { 
			tgt.scrub.icn.red._visible = false;
		};
		tgt.scrub.icn.onMouseDown = function() {
			tgt.scrub.icn.red._visible = true;
		};	
		tgt.scrub.icn.onMouseUp = function() {
			tgt.scrub.icn.red._visible = false;
		};	
		...
		/* download button link (logo button really) */
		tgt.dl.onPress = function() {
			getURL("http://elgg.com", "_blank");
		};
	...
	private function setDimensions() {
		...
		if(feeder.feed.length - feeder.numads == 1 ||
			...
			tgt.scrub.shd._width = cbw-21; /* width of play button */
			tgt.scrub._x = 21;
		} else {
		...
		var xp = cbw;
		/* swopped for campuslogo */
		if (config["showdownload"] == "true") {
			xp -= 40; /* was 68 for campus */
			tgt.scrub.shd._width -= 40; /* was 68 for campus */
			tgt.dl._x = xp;
		} else {
		...
		if((Stage["displayState"] == undefined ||
			...
			xp -= 21;
			tgt.scrub.shd._width -= 21;
			tgt.fs._x = xp;
		}
		
		if(config["showvolume"] == "true") {
			xp -= 37;
			tgt.scrub.shd._width -= 38; /* extra pixel added here to create gap before volume box */
			tgt.vol._x = xp;
		}
		...
		if(config["showdigits"] == "false" || tgt.scrub.shd._width < 120 ||
			...
			tgt.scrub.bar._x = tgt.scrub.bck._x = tgt.scrub.icn._x = 5;
			barWidths = tgt.scrub.bck._width = tgt.scrub.shd._width - 80; /* -80 for text strings space */
			tgt.scrub.remTxt._x = tgt.scrub.shd._width - 34;
			tgt.scrub.elpTxt._x = tgt.scrub.remTxt._x - 39;
		}	
	...
	private function setTime(elp:Number,rem:Number) {
		...
		tgt.icn._x = tgt.bar._width + tgt.bar._x;
