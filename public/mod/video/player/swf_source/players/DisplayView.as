...
class com.elggmediaplayer.players.DisplayView extends AbstractView { 
...
/*
		tgt.playicon.col = new Color(tgt.playicon.playtriangle);
		//tgt.playicon.col.setRGB(config["scrubicncolor"]);		
		tgt.playicon.onRollOver = function() { 
			tgt.playicon.col.setRGB(ref.config["lightcolor"]);
		};
		tgt.playicon.onRollOut = function() { 
			tgt.playicon.col.setRGB(ref.config["frontcolor"]);
		};
*/
/*
		tgt.playicon.onMouseDown = function() {
			tgt.playicon.col.setRGB(ref.config["lightcolor"]);
		};	
		tgt.playicon.onMouseUp = function() {
			tgt.playicon.col.setRGB(ref.config["frontcolor"]);
		};
*/			
		...
		if(config["logo"] != "undefined") {
			...
			lll.onLoadFinished = function() {
				...
				tgt.logo._x	= ref.config["displaywidth"] - (ref.config["displaywidth"] - 10);
	...
	private function setDimensions() {
		...
		if(Stage["displayState"] == "fullScreen") {
			...
			tgt.logo._x	= Stage.width - (Stage.width - 20);
			tgt.logo._y = 20;
		} else {
			tgt.playicon._xscale = tgt.playicon._yscale = 
				tgt.muteicon._xscale = tgt.muteicon._yscale =
			...
			if(tgt.logo._height > 1) {
				...
				tgt.logo._x	= config["displaywidth"] - (config["displaywidth"] - 10);
				tgt.logo._y = 10;
			}
		}
	};
